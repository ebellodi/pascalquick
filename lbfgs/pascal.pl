...
default_setting_pascal(beamsize,2).
...
default_setting_pascal(min_accuracy,0.65).
default_setting_pascal(max_nodes,20). %max num iterazioni findBestIC
...
default_setting_pascal(max_iter,10). %iter clause search
default_setting_pascal(max_iter_structure,100). %iter theory search
/* default_setting_pascal(max_lengths[Body,Disjucts,LitIn+,LitIn-]). */
default_setting_pascal(max_lengths,[2,2,2,2]).
...
test_pascal(P,TestFolds,LL,AUCROC,ROC,AUCPR,PR):-
  ...
  format("~nRanking test examples~n"), format("~p~n~n",[LG]),
...
induce_par_int(Folds,M,DB,Program):-
  ...
  length(Pos,NP),
  length(Neg,NN),
  format("/* Inizio l'apprendimento dei pesi, N pos ~d N neg ~d */",[NP,NN]),nl,
  learn_param(Program0,M,Pos,Neg,Program,LL),
  format("/* Log likelihood ~f~n*/~n",[LL]),
  print_list1(Program),
  retract_all(BGRefs).
...
%    load_models(FileKB,HB,ModulesList),
  ...
  length(Pos,NP),
  length(Neg,NN),
  format("/* Inizio l'apprendimento, N pos ~d N neg ~d */~n",[NP,NN]),
  induce(Pos,Neg,M,Program0),
  %remove_red(Pos,ProgramRed,[],Program0),
  length(Program0,N0),
  format("~nLunghezza programma = ~d~n",[N0]),
  ...
  print_list(Program0),
  change_format(Program0,Program1),
  ...
  format("~n/* Learning time ~f seconds. Generated rules */~N",[T1]),
  format("/* Number of rules ~d */~n",[N1]),
  format("/* ~p */~n~n",[L]),    
 % format("/* Negative examples remaining: ~d~n~p~n*/~n",[NR,NegRem]),   
  format("/* Language bias ~n~p~n*/~n",[optimal(Opt)]),
  format("/* Log likelihood ~f~n*/~n",[LL]),
  print_list1(Program),
  retract_all(BGRefs).
%clear_kb(HB).
...
change_format([Rule|Pr0], [RuleProb|Pr1]):-
		%	Rule = rule(r, Clause, _Stat),
		Rule = (r, Clause, _Stat),
		RuleProb = rule(r, Clause, 'p'),
		change_format(Pr0,Pr1).
...
induce(Pos,Neg,M,Program):-
    prior_prob(Pos,Neg,M,NP,NN),
	manage_modex(M), %asserisce i modeh/b
	BestClause  = (null,([], []:-[], []),(0,0,_,_,_)), %(Name,Clause,(H,NN,NP,Emc,Epnc))
%findBestICS(Clauses,M,Ep,Em,NPT,NNT,N,BIn,BOut):-
	findBestICS([BestClause],M,Pos,Neg,NP,NN,1,[],Program).
...
manage_modex(M):-
		...
		cycle_modex(BL0,M,'modeb'),
		get_modeh(M,HL0),
		%flatten_multiple_var_modex(HL0,HL),
	    cycle_modex(HL0,M,'modeh').
...
cycle_modex([(A,P)|T],M,Type):-
	P=..[F|Args],
	count_values(Args,NL),
	NL>0,!,
	ModeR=..[Type,A,P],
	retract(M:ModeR),!,
	findall(Modex,create_new_modex(Type,M,A,F,Args,Modex),_),
	cycle_modex(T,M,Type).
...
create_new_modex(Type,M,A,F,Args,Modex):-
	length(Args,N),
	length(Args1,N),
	P=..[F,_|Args1],
	call(M:P),
	replace_values(Args1,Args,Args2),
	NewP=..[F|Args2],
	Modex=..[Type,A,NewP],
	\+ call(M:Modex),
	assert(M:Modex).
...
replace_values([H|T1],[-#_|T],[H|T2]):-
	!,
	replace_values(T1,T,T2).
	
replace_values([H|T1],[#_|T],[H|T2]):-
	!,
	replace_values(T1,T,T2).

replace_values([_|T1],[H|T],[H|T2]):-
	!,
	replace_values(T1,T,T2).
...
%covering_loop1(_Pos,[],_,[],_NP,_NN,_NR,_NR2,[]):-!.

/* some eminus still to cover: generate a new clause */
%covering_loop1(Eplus,Eminus,M,NP,NN,BeamIn,BeamOut):-
...
% INPUT
% findBestICS([BestClause],M,Eplus,Eminus,NP,NN,0,[],Program).
findBestICS([],_M,_Ep,_Em,_NPT,_NNT,_N,BOut,BOut):-!.

findBestICS(_,M,_Ep,_Em,_NPT,_NNT,N,BOut,BOut):-
		M:local_setting(max_nodes,NMax), %max num iterazioni 
        N>NMax,!.

%Clauses = { true --> false }
%NewBeam = i raffinamenti di BeamIn ordinati per euristica
%BIn = il beam iniziale o allo step precedente
%BOut = beam complessivo da restituire
findBestICS(Clauses,M,Ep,Em,NPT,NNT,N,BIn,BOut):-
	...
	format("~n** Iterazione ~d **~n",[N]),
	generate_new_agenda1(Ep,Em,M,NPT,NNT,Clauses,BIn,NewBeam),
	%	append(BIn,NewBeam,BIn1),
	N1 is N+1,!,
	%    findBestICS(NewAgenda,Ep,Em,NPT,NNT,N1,BC1,BCOut).
	findBestICS(NewBeam,M,Ep,Em,NPT,NNT,N1,NewBeam,BOut).

%non ci sono regole da raffinare
generate_new_agenda1(_Ep,_Em,_M,_NPT,_NNT,[],NewB,NewB):-!.    

%la lista [Rule0|Rest] e' da raffinare
%NBIn Beam corrente
%LRef+NBIn=NB1 ordinato
generate_new_agenda1(Ep,Em,M,NPT,NNT,[Rule0|Rest],NBIn,NewBeam):-
	%    findall(NewClause,refine(Clause, NewClause),Ref),
	Rule0=(N,R0,P),
	Rule=rule(N,R0,P),
	%format("Revision of one clause ~n",[]),	
	findall(RS, generalize_theory([Rule],M,RS),LRef), %LRef=lista di liste, 1 per clausola raffinata
	%	write(LRef),nl,
    evaluate_all_refinements(Ep,Em,M,NPT,NNT,LRef,NBIn,NB1),!,
   ...
   %evaluate_all_refinements(Ep,Em,NPT,NNT,[HRef|TRef],Name,NAgIn,NAgOut,(NameIn,BCIn,HeurIn,DetIn),(NameOut,BCOut,HeurOut,DetOut)):-
    generate_new_agenda1(Ep,Em,M,NPT,NNT,Rest,NB1,NewBeam).


/*generate_new_agenda1(_Ep,_Em,_M,_NPT,_NNT,[],NewAg,NewAg):-!.    

generate_new_agenda1(Ep,Em,M,NPT,NNT,[Rule0|Rest],NAgIn,NAgOut):-
	Rule0=(N,R0,P),
	Rule=rule(N,R0,P),
	format("Generalization ~n",[]),	
	findall(RS, generalize_theory([Rule],M,RS),LRef),
    (LRef = [] ->	
	    generate_new_agenda1(Ep,Em,M,NPT,NNT,Rest,NAgIn,NAgOut)
	;
		evaluate_all_refinements(Ep,Em,M,NPT,NNT,LRef,NAgIn,NAg1),
		generate_new_agenda1(Ep,Em,M,NPT,NNT,Rest,NAg1,NAgOut)
	).
*/
...
specialize_rule_body([Lit|_RLit],(H:-B),M,(H:-BL1)):-  %Lit contiene modeb
  ...
  specialize_lit([Lit|LLitOut],M,ALL,[SLit]),
  append(B,[SLit],BL1),
...
specialize_lit_list([Lit|RLits],M,TypeVars,[SLit|RSLits]):-
  Lit =.. [Pred|Args],
     ...
     (
       ...
       (  % genera +
	    (
	      ...
	      member(Disj,R),
	      specialize_lit(Disj,M,ALL,SLits),
	      append(H,[(+,SLits,[])],HL1),
...
evaluate_all_refinements(_Ep,_Em,_M,_NPT,_NNT,[],NBOut,NBOut):-!.

%evaluate_all_refinements(Ep,Em,M,NPT,NNT,LRef,NBIn,NB1),!,
%NBIn=beam corrente, NBOut=beam aggiornato
evaluate_all_refinements(Ep,Em,M,NPT,NNT,[[HRef]|TRef],NBIn,NBOut):-
	HRef=rule(Name,HRef1,_Stat),
    generate_query(HRef1,Query,VI),
    test_clause_pos(Ep,M,Query,VI,0,NP,[],Epc), %NP=num pos coperti da Query; Epc=lista pos coperti (lunga NP)
	%	format("num pos cov = ~d~n",[NP]),
    test_clause_neg(Em,M,Query,VI,0,NN,[],Emc), %NN=num neg dove Query è falsa; EMC=lista neg corrispondenti 
	%format("num neg cov = ~d~n",[NN]),
    deleteall(Ep,Epc,Epnc), %Epnc=lista pos NON coperti
    M:local_setting(min_coverage,MC),
    M:local_setting(min_accuracy,MA),
    (M:local_setting(heur,laplace)->
    ...
    ;
      ...
      (Den=0->
        Heuristic is 0     
      ;         
        Heuristic is (NN)/Den   %heuristic is computed as the number of ruled out negative traces over the total number of ruled out traces (positive and negative)
      )
    ),
	(( Heuristic>=MA,NN>=MC)->
       % Det1 = (NN,NP,Emc,Epnc),
	   %	append([(Name,HRef1,Heuristic,Det1)],AllRefinementsIn,AllRefinementsIn1)
        ...
        print_ref(Name,M,HRef,Heuristic,NN,NP,Emc,Epnc),
        insert_in_order((Name,HRef1,Heuristic,NN),BS,NBIn,NB1)

    ;
		NB1=NBIn
    ),!,
    evaluate_all_refinements(Ep,Em,M,NPT,NNT,TRef,NB1,NBOut).
...
insert_in_order((Name,HRef,Heuristic,NN),BeamSize,
        ...
        BeamOut):-
    (Heuristic>Heuristic1;(Heuristic=Heuristic1,NN>NN1)),!,
...
get_pos_neg(DB,Mod,Pos,Neg):-
  ...
  findall(M,(member(M,DB),Mod:AtomP),Pos),
...
print_list([(_Name,C,Stat)|Rest]):-
    numbervars(C,0,_M),
    write_clause(C),
	format("/* Heuristic, NegEx ruled out: ~p */~n~n",[Stat]),    
	%format("/* P = ~p */~n~n",[Stat]),    
    print_list(Rest).
...
%input  (r, ([], []:-[p6_polar(_1230), greater2Po(..., ...)], []), 1, 6)
learn_param(Program0,M,Pos,Neg,Program,NewL1):-
  M:local_setting(learning_algorithm,lbfgs),!,  
  M:local_setting(verbosity,V),
  ( V > 2 ->
        ( format("~n~nPARAMETER LEARNING LBFGS~n",[]) )
    ;
         true
  ),
  convert_prob(Program0,Pr1),%converte le regole di Program0 in query (congiunzioni di atomi) da usare in test_theory_pos/neg_prob
%  gen_par(0,NC,Par0),
  length(Program0,N),
  length(Pos,NPos),
  length(Neg,NNeg),
...
/*  R0 is R*(-1),
  ...
  format("valore random iniziale parametri: ~f",[R1]),nl,
  init_par(N,R1),*/
  evaluate_L(MIP,MI,M,L),
  IL is -L,
  ( V > 2 ->
        ( format("~nInitial L ~f~n",[IL]) )
    ;
         true
  ),
  ...
  ( V > 2 ->
      ...
      format("Status ~p ~s~n",[Status,Mess])
           ;
         true
  ),
...
learn_param(Program0,M,Pos,Neg,Program,NewL1):-
  ...
  M:local_setting(verbosity,V),
  ( V > 2 ->
        ( format("~n~n*** learn_param gradient descent***~n",[]) )
    ;
         true
  ),
...
initial_weights(N,R):-
  R0 is R*(-1),
  random(R0,R,W),  %genera val random in (-1,1)
  format("parametro ~d: ~f",[N,W]),nl,
  init_par(N,W),
  N1 is N-1,
  initial_weights(N1,R).
...
random_restarts(N,NR,M,MIP,MI,NEx,Score0,Score,Par0,Par):-
  ...
  gradient_descent(0,Iter,M,W,MIP,MI,NEx,NR),
...
initialize_weights(NR,M,W):-
  ...
  maplist(random_weight(MW),WA).

random_weight(MW,W):-
  Min is -MW,
  random(Min,MW,W).

gradient_descent(I,I,_,_,_MIP,_MI,_NEx,_NR):-!.

gradient_descent(Iter,MaxIter,M,W,MIP,MI,NEx,NR):-
  evaluate_w(MIP,MI,W,M,LN,LL),
  duplicate_term(W,WC),
  format2(M,"Gradient descent iteration ~d, LL ~f~n",[Iter,LL]),
  length(GA,NR),
  G=..[g|GA],
  maplist(g_init,GA),
  writeln(G),
  compute_grad_w(MIP,W,G,1,MI,M,LN),    
  writeln(W),
  writeln(G),
  learning_rate(M,Iter,Eta),
  format2(M,"Learning rate ~f~n",[Eta]),  
  nl,
  update_weights(W,G,NEx,Eta),
  Iter1 is Iter+1,
  assertz(M:p(WC,LL)),
  gradient_descent(Iter1,MaxIter,M,W,MIP,MI,NEx,NR).
...
update_weights(W,G,NEx,Eta):-
  functor(W,_,NR),
  numlist(1,NR,L),
  maplist(update_w(W,G,NEx,Eta),L).

update_w(W,G,NEx,Eta,NR):-
  arg(NR,G,G0),
  arg(NR,W,W0),
  New_W0 is W0-Eta*G0/NEx,
  setarg(NR,W,New_W0).
...
evaluate(L,N,_Step,M,MIP,MI,NEx):-
		...
		%        %  NL is -L,
		%        %  write(l),nl,
   		compute_grad(MIP,0,MI,M,NEx,LN),
	    store_hist(M,N,L).
...
compute_grad_w([],_W,_G,_N,_MI,_M,_LN):-!.

compute_grad_w([HMIP|TMIP],W,G,N0,MI,M,LN):-
  N00 is N0-1,
  compute_sum_neg(MI,LN,N00,M,0,S),
  arg(N0,W,W0),
%  optimizer_get_x(N0,W0),
  G0 is (HMIP-S)*(1/(1+exp(-W0))),
  setarg(N0,G,G0),
  format("~ngradiente:",[]),write(G),nl,
  %  optimizer_set_g(N0,G),
  N1 is N0+1,
  compute_grad_w(TMIP,W,G,N1,MI,M,LN).
...
compute_grad([],_N,_MI,_M,_NEx,_LN):-!.

compute_grad([HMIP|TMIP],N0,MI,M,NEx,LN):-
  compute_sum_neg(MI,LN,N0,M,0,S),
  optimizer_get_x(N0,W0),
  G is (HMIP-S)*(1/(1+exp(-W0)))/NEx,
  format("~ngradiente:",[]),write(G),nl,
  optimizer_set_g(N0,G),
  N1 is N0+1,
  compute_grad(TMIP,N1,MI,M,NEx,LN).
...
print_ref(_Name,M,C,Heur,NC,PC,_Emc,_Epnc):-
        M:local_setting(verbosity,V),
        V>0,
        format("Refinement:~n",[]),
		C = rule(r,C1,_),
